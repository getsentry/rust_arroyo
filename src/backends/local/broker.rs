...
use crate::backends::storages::{MessageStorage, TopicExists, TopicDoesNotExist, ConsumeError, PartitionDoesNotExist};
...
impl<TPayload: Clone> LocalBroker<TPayload> {
    ...
    pub fn produce(
        ...
        self.storage.produce(partition, payload, DateTime::from(time))
    }
    ...
    pub fn subscribe(
        ...
        if group_subscriptions.is_some() {
            let consumer_subscription = group_subscriptions
                .unwrap()
                .get(&consumer_id);
            if consumer_subscription.is_none() {
                ...
                let mut non_matches = subscribed_topics.iter()
                    .zip(&topics).filter(|&(a, b)| a.name != b.name);
        ...
        match group_subscriptions {
            None => {
                ...
                self.subscriptions.insert(consumer_group.clone(), new_group_subscriptions);
            }
    ...
    pub fn unsubscribe(
        ...
        for topic in subscribed_topics.iter() {
            ...
            for n in 0..partitions {
                ret_partitions.push(Partition{topic: topic.clone(), index: n});
            }
        }
...
mod tests {
    ...
    fn test_produce_consume() {
        ...
        let _ = broker.create_topic(Topic {name: "test".to_string()}, 1);
    ...
    fn test_assignment() {
        ...
        let r_assignments = broker.subscribe(Uuid::nil(), "group".to_string(), vec![topic1.clone(), topic2.clone()]);
