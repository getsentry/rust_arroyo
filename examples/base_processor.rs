...
use rust_arroyo::types::{Message, Partition, Topic, Position};
...
use rust_arroyo::processing::{StreamProcessor, RunError};
...
impl ProcessingStrategy<OwnedMessage> for TestStrategy {
    fn poll(&mut self) -> Option<CommitRequest> {
        ...
        if self.partitions.len() > 0 {
            ...
            // to work now.
            let ret = Some(CommitRequest{positions: self.partitions.clone()});
...
impl ProcessingStrategyFactory<OwnedMessage> for TestFactory {
    fn create(
        &self, 
    ) -> Box<dyn ProcessingStrategy<OwnedMessage>> {
        Box::new(TestStrategy{partitions: HashMap::new()})
    }
}
